...
public class Util {
    ...
    public static String handleBalance(String balance, int pointNum){
        ...
        return formatDecimal(d / 100000000, pointNum) + "亿";
    }

    public static String handleBalance(double balance, int pointNum){
        return handleBalance(balance + "", pointNum);
    }

    /**
     * 银行卡号转*号
     */
    public static String transferCard2Star(String cardNum) {
        StringBuffer number = new StringBuffer();
        int endCount = cardNum.length() % 4;
        if (endCount == 0) {
            endCount = 4;
        }
        for (int i = 0; i < cardNum.length(); i++) {
            if (i < cardNum.length() - endCount) {
                number.append("*");
                if ((i + 1) % 4 == 0) {
                    number.append(" ");
                }
