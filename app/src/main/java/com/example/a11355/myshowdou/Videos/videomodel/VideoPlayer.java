package com.example.a11355.myshowdou.Videos.videomodel;
...
import com.example.a11355.myshowdou.Videos.videomodel.utils.LightUtil;
...
import static com.example.a11355.myshowdou.Videos.videomodel.utils.PlayerUtil.dip2px;
...
public class VideoPlayer extends FrameLayout implements View.OnClickListener,
    ...
    private void unLockScreen() {
        isLockScreen = false;
        playerLock.setImageResource(R.mipmap.player_lock_open);
    }

    private void lockScreen() {
        isLockScreen = true;
        playerLock.setImageResource(R.mipmap.player_lock_close);
    }
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (Intent.ACTION_BATTERY_CHANGED.equals(intent.getAction())) {
                ...
                if (battery > 0 && battery < 20) {
                    ivBattery.setImageResource(R.mipmap.player_battery_01);
                } else if (battery >= 20 && battery < 40) {
                    ivBattery.setImageResource(R.mipmap.player_battery_02);
                } else if (battery >= 40 && battery < 65) {
                    ivBattery.setImageResource(R.mipmap.player_battery_03);
                } else if (battery >= 65 && battery < 90) {
                    ivBattery.setImageResource(R.mipmap.player_battery_04);
                } else if (battery >= 90 && battery <= 100) {
                    ivBattery.setImageResource(R.mipmap.player_battery_05);
                } else {
                ...
                if (mediaPlayer != null && mediaPlayer.isPlaying()) {
                    if (Math.abs(distanceX) > Math.abs(distanceY)) {// 横向移动大于纵向移动
                        if (distanceX >= dip2px(context, STEP_PROGRESS)) {// 快退，用步长控制改变速度，可微调
                            gesture_iv_progress.setImageResource(R.mipmap.player_backward);
                            ...
                            gesture_iv_progress.setImageResource(R.mipmap.player_forward);
        ...
        // 如果每次触摸屏幕后第一次scroll是调节音量，那之后的scroll事件都处理音量调节，直到离开屏幕执行下一次操作
            ...
            if (Math.abs(distanceY) > Math.abs(distanceX)) {// 纵向移动大于横向移动
                if (currentVolume == 0) {// 静音，设定静音独有的图片
                    gesture_iv_player_volume.setImageResource(R.mipmap.player_volume_close);
                }
                if (distanceY >= dip2px(context, STEP_VOLUME)) {// 音量调大,注意横屏时的坐标体系,尽管左上角是原点，但横向向上滑动时distanceY为正
                    ...
                    gesture_iv_player_volume.setImageResource(R.mipmap.player_volume_open);
        ...
        //调节亮度
            ...
            if (Math.abs(distanceY) > Math.abs(distanceX)) {// 纵向移动大于横向移动
                ...
                if (mLight >= 0 && mLight <= 255) {
                    ...
                    } else if (distanceY <= -dip2px(context, STEP_LIGHT)) {// 亮度调小
                        if (mLight < 10) {
